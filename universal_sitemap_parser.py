...
class SourceConfig:
    ...
    def root_sitemap_urls(self) -> List[str]:
        """Return one or more root sitemap URLs from the config cell.

        A Google Sheet cell may contain multiple sitemap URLs separated by line
        breaks. Each URL is treated as an independent root while keeping one
        source_id/config row.
        """
        roots: List[str] = []
        seen: Set[str] = set()

        for raw in self.sitemap_url.splitlines():
            url = raw.strip()
...
class SitemapFetcher:
    ...
    def _looks_like_sitemap(data: bytes) -> bool:
        sample = data[:65536].lstrip(b"\xef\xbb\xbf \r\n\t")
        return bool(
            re.search(
                br"<(?:[\w.-]+:)?(?:urlset|sitemapindex)(?:\s|>)",
                sample,
                re.I,
            )
        )
    ...
    @classmethod
    def _looks_like_challenge(cls, status: Optional[int], data: bytes) -> bool:
        if status not in {403, 429, 503}:
            return False
        text = data[:200000].decode("utf-8", errors="ignore").lower()
        return any(marker in text for marker in cls.CHALLENGE_MARKERS)

    def _request_once(self, url: str, browser_profile: bool = False) -> bytes:
        headers = None
        if browser_profile:
            headers = {
                **self.BROWSER_HEADERS,
                "User-Agent": self.BROWSER_USER_AGENT,
                **self.cfg.request_headers,
            }
    ...
    def _copy_browser_cookies_to_requests(self) -> None:
    ...
    def _browser_response_bytes(self, url: str) -> Tuple[Optional[int], bytes]:
        ...
        return response.status, self._decode_gzip(body)

    def _fetch_browser(self, url: str) -> bytes:
        print(f"HTTP sitemap fetch failed; trying Chromium: {url}")

        status, data = self._browser_response_bytes(url)

        wait_seconds = max(
            0,
            int(os.environ.get("AUTOMATIC_CHALLENGE_WAIT_SECONDS", "12") or 12),
        )

        if self._looks_like_challenge(status, data) and wait_seconds:
            assert self._page is not None
            print(
                f"Chromium received HTTP {status}; waiting up to "
                f"{wait_seconds}s for an automatic challenge: {url}"
            )
            ...
            while time.time() < deadline:
                ...
                combined = title + "\n" + html
                if not any(marker in combined for marker in self.CHALLENGE_MARKERS):
                    break
        ...
        if status is None:
            raise RuntimeError("Chromium returned no HTTP response.")

        if not (200 <= status < 300):
            raise RuntimeError(f"Chromium returned HTTP {status}.")

        if not self._looks_like_sitemap(data):
            raise RuntimeError(
                f"Chromium returned HTTP {status}, but the response is not sitemap XML."
            )

        self._copy_browser_cookies_to_requests()
        # Once browser access succeeds, use the same browser-like UA for child
        # sitemap requests. This also preserves any cookies set by the site.
        self.session.headers.update(
            {
                **self.BROWSER_HEADERS,
                "User-Agent": self.BROWSER_USER_AGENT,
    ...
    def fetch_bytes(self, url: str) -> bytes:
        ...
        for attempt in range(1, self.cfg.retries + 1):
            try:
                return self._request_once(url, browser_profile=False)
            except Exception as exc:
                errors.append(f"requests attempt {attempt}: {exc}")
        ...
        # A browser-like raw HTTP request is cheap and fixes servers that reject
        # the engine's configured bot-style User-Agent but do not require JS.
        try:
            return self._request_once(url, browser_profile=True)
        except Exception as exc:
            errors.append(f"browser-profile requests: {exc}")
        ...
        raise RuntimeError(
            f"Failed to fetch sitemap {url}: " + " | ".join(errors[-6:])
        )
...
def discover_leaf_sitemaps(
    ...
    def walk(url: str, depth: int, is_root: bool, root_url: str) -> None:
        if url in visited:
            return
        visited.add(url)
        ...
        for entry in entries:
            ...
            if child_url:
                walk(child_url, depth + 1, False, root_url)

    roots = source.root_sitemap_urls()
    if not roots:
        return [], ["No valid root sitemap URL configured."]

    for root_url in roots:
        walk(root_url, 0, True, root_url)
...
def make_state_row(
    ...
    return [
        ...
        source.source_id,
        source.site_url,
        leaf.root_url or source.sitemap_url,
        leaf.url,
        len(urls),
        urls[-1] if urls else "",
